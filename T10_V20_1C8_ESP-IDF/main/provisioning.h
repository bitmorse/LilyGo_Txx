...
// --- credentials (written over BLE by the app; verified live by netmgr) ---
// Store candidate STA credentials (persists to NVS). netmgr then attempts to connect
// and only treats the device as "provisioned" on a confirmed GOT_IP; on failure it
// calls provisioning_forget(). Returns false on a bad/empty SSID or a config error.
...
const char *provisioning_service_name(void);   // device id "T10_XXXXXX"
