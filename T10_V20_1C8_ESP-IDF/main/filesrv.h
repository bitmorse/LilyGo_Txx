...
// Mint a fresh session token without starting the server, and return it. Lets the
// caller hand the token to the phone over BLE and tear BLE down BEFORE starting the
// (heap-heavy) HTTP server; filesrv_start() then reuses this token.
const char *filesrv_new_token(void);
